...
function ProductDetailShort(props) {
  return (
    <View style={style.ProductDetailShort} testID="ProductDetailShort">
        <View>
          <Text>Action :</Text>
          <Button title="close" color='lightblue' onPress={()=>{props.action.close()}}/>
          <Button title="Edit" color="orange"/>
          <Button title="Add" color="lightgreen" onPress={()=>{props.action.add()}}/>
        </View>
        <View>
          <Image source={{uri:props.product.img}} style={style.leftImage} resizeMode="center"/>
        </View>
        <View>
